...
void Combat::registerActions(vector<Character *>::iterator participantIterator) {
    ...
    while (participantIterator != participants.end()) {
        if ((*participantIterator)->getIsPlayer()) {
            vector<Character*> characterEnemies(enemies.begin(), enemies.end());
            Action playerAction = ((Player *) *participantIterator)->takeAction(characterEnemies);

            actionQueue.push(playerAction);
        } else {
            vector<Character*> characterPartyMembers(partyMembers.begin(), partyMembers.end());
            Action enemyAction = ((Enemy *) *participantIterator)->takeAction(characterPartyMembers);
